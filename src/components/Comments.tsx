...
type CommentDoc = Models.Document & {
    content: string;
    authorId: string;
    author?: { name: string; [k: string]: any } | null;
    $createdAt?: string;
};

function Comments({
    comments: _comments, type, typeId, className,
}: {
    ...
    const [comments, setComments] = React.useState<Models.DocumentList<CommentDoc>>(_comments);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setNewComment(() => "");
            const newDoc = { ...(response as any), author: user } as CommentDoc;
            setComments(prev => ({
                total: prev.total + 1,
